...
import { Drawer, JoinedClasses, Login, Main , Watch} from "./components";
import { BrowserRouter as Router, Switch, Route } from "react-router-dom";
import { IsUserRedirect, ProtectedRoute } from "./routes/Routes";
import { useLocalContext } from "./context/context";
import db from "./lib/firebase";
function App() {
  const { loggedInMail , details} = useLocalContext();
  ...
  return (
    <Router>
      <Switch>
          {details.map((item) => (
              <Route path={`/Watch/${item.detailid}`} key={item.detailid}>
                <Watch detail={item} />
              </Route>
            ))}
